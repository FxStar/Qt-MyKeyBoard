...
Keyboard::Keyboard(QWidget *parent) : AbstractKeyboard(parent),
                                      m_isChinese(false)
{
...
void Keyboard::switchEnOrCh()
{
    ...
    // 在此处添加逻辑以根据 m_isChinese 变量来显示/隐藏 m_chineseWidget
    int chineseWidgetHeight = m_chineseWidget->height();
    if (m_isChinese)
    {

        // 获取整个布局的高度
        int layoutHeight = this->height();
        // 设置整个布局的高度为初始高度加上中文键盘的高度
        this->setFixedHeight(layoutHeight + chineseWidgetHeight);
...
ChineseWidget::ChineseWidget(QWidget *parent) : QListWidget(parent)
{
